...
namespace net.esper.compat
{
    ...
    public class ReaderLock : BaseLock
    {
        ...
        /// <param name="lockObj">The lock obj.</param>
        public ReaderLock(String name, ReaderWriterLock lockObj)
        {
        ...
        public override void Dispose()
        {
            lock (this)
            {
                ...
                {
                    if ((this.m_uLockObj != null) && (this.m_uLockObj.IsReaderLockHeld))
                    {
                        if (ThreadLogUtil.ENABLED_TRACE)
                        {
                            ThreadLogUtil.TraceLock(RELEASE_TEXT + " read " + m_uLockName, m_uLockObj);
                        }
                        ...
                        if (ThreadLogUtil.ENABLED_TRACE)
                        {
                            ThreadLogUtil.TraceLock(RELEASED_TEXT + " read " + m_uLockName, m_uLockObj);
                        }
                    }

                    if ((this.m_mLockObj != null) && (this.m_mLockObj.IsReaderLockHeld))
                    {
