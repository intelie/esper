...
public class TestUnidirectionalStreamJoin extends TestCase
{
    ...
    public void testInvalid()
    {
        String text = "select * from " + SupportBean.class.getName() + " unidirectional " +
                          ...
                          "on string = symbol";
        tryInvalid(text, "Error starting view: The unidirectional keyword can only apply to one stream in a join [select * from net.esper.support.bean.SupportBean unidirectional full outer join net.esper.support.bean.SupportMarketDataBean.win:keepall() unidirectional on string = symbol]");

        text = "select * from " + SupportBean.class.getName() + ".win:length(2) unidirectional " +
                          "full outer join " +
                          SupportMarketDataBean.class.getName() + ".win:keepall()" +
                          "on string = symbol";
        tryInvalid(text, "Error starting view: The unidirectional keyword requires that no views are declared onto the stream [select * from net.esper.support.bean.SupportBean.win:length(2) unidirectional full outer join net.esper.support.bean.SupportMarketDataBean.win:keepall()on string = symbol]");
    }
