        ...
        throws ExprValidationException, ViewProcessingException
    {
        ...
        if (streamSpec instanceof FilterStreamSpecCompiled)
        {
            ...
            Pair<EventStream, ManagedLock> streamLockPair = services.getStreamService().createStream(statementContext.getStatementId(), filterStreamSpec.getFilterSpec(),
                    statementContext.getFilterService(), statementContext.getEpStatementHandle(), false, false, statementContext);
        ...
        if (statementSpec.getOnTriggerDesc() instanceof OnTriggerWindowDesc)
        {
            ...
            onExprView = processor.addOnExpr(onTriggerDesc, validatedJoin, streamEventType, statementContext.getStatementStopService(), routerService, resultSetProcessor, statementContext.getEpStatementHandle(), statementContext.getStatementResultService(), statementContext);
        ...
        throws ExprValidationException, ViewProcessingException
    {
        ...
        for (int i = 0; i < statementSpec.getStreamSpecs().size(); i++)
        {
            ...
            if (streamSpec instanceof FilterStreamSpecCompiled)
            {
                ...
                // Since only for non-joins we get the existing stream's lock and try to reuse it's views
                Pair<EventStream, ManagedLock> streamLockPair = services.getStreamService().createStream(statementContext.getStatementId(), filterStreamSpec.getFilterSpec(),
                        statementContext.getFilterService(), statementContext.getEpStatementHandle(), isJoin, false, statementContext);
            ...
            throws ExprValidationException, ViewProcessingException
    {
        ...
        for (ExprSubselectNode subselect : statementSpec.getSubSelectExpressions())
        {
            ...
            if (statementSpec.getStreamSpecs().get(0) instanceof FilterStreamSpecCompiled)
            {
                ...
                Pair<EventStream, ManagedLock> streamLockPair = services.getStreamService().createStream(statementContext.getStatementId(), filterStreamSpec.getFilterSpec(),
                        statementContext.getFilterService(), statementContext.getEpStatementHandle(), isJoin, true, statementContext);
            ...
            if (selectClauseSpec.getSelectExprList().size() > 0)
            {
                ...
                if (element instanceof SelectClauseExprCompiledSpec)
                {
                    ...
                    if (aggExprNodes.size() > 0)
                    {
                        ...
                        aggregationService = AggregationServiceFactory.getService(aggExprNodes, havingAgg, orderByAgg, false, null, statementContext);
