...
public class TestSubselectWithinPattern extends TestCase {
    ...
    public void testInvalid() {
        ...
        tryInvalid("select * from S0(exists (select * from S1))",
                "Subqueries require one or more views to limit the stream, consider declaring a length or time window [select * from S0(exists (select * from S1))]");
        ...
        tryInvalid("select * from S0(exists (select * from MyWindow.std:lastevent()))",
                "Consuming statements to a named window cannot declare a data window view onto the named window [select * from S0(exists (select * from MyWindow.std:lastevent()))]");

        tryInvalid("select * from S0(id in ((select p00 from MyWindow)))",
                "Implicit conversion not allowed: Cannot coerce types java.lang.Integer and java.lang.String [select * from S0(id in ((select p00 from MyWindow)))]");
    }
