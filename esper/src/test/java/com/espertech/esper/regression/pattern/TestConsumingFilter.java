...
public class TestConsumingFilter extends TestCase implements SupportBeanConstants
{
    ...
    public void testInvalid() {
        tryInvalid("select * from pattern[every a=SupportBean@consume()]", "Incorrect syntax near ')' at line 1 column 50, please check the filter specification within the pattern expression within the from clause [select * from pattern[every a=SupportBean@consume()]]");
        tryInvalid("select * from pattern[every a=SupportBean@consume(-1)]", "Incorrect syntax near '-' at line 1 column 50, please check the filter specification within the pattern expression within the from clause [select * from pattern[every a=SupportBean@consume(-1)]]");
        tryInvalid("select * from pattern[every a=SupportBean@xx]", "Error in expression: Unexpected pattern filter @ annotation, expecting 'consume' but received 'xx' [select * from pattern[every a=SupportBean@xx]]");
    }
