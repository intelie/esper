...
public class TestFirstLastAllAggregation extends TestCase {
    ...
    public void testFirstLastIndexedWithAnonymousFields() {
        String epl = "select " +
                "last(intPrimitive), " +
                "last(intPrimitive, 1)," +
        ...
        EPStatement stmt = epService.getEPAdministrator().createEPL(epl);
        stmt.addListener(listener);
        String[] fields = new String[]{"last(intPrimitive)", "last(intPrimitive, 1)", "first(intPrimitive, 1)"};

        epService.getEPRuntime().sendEvent(new SupportBean("E1", 10));
        ArrayAssertionUtil.assertProps(listener.assertOneGetNewAndReset(), fields, new Object[]{10, null, null});

        epService.getEPRuntime().sendEvent(new SupportBean("E2", 11));
        ArrayAssertionUtil.assertProps(listener.assertOneGetNewAndReset(), fields, new Object[]{11, 10, 11});

        epService.getEPRuntime().sendEvent(new SupportBean("E3", 12));
        ArrayAssertionUtil.assertProps(listener.assertOneGetNewAndReset(), fields, new Object[]{12, 11, 12});

        epService.getEPRuntime().sendEvent(new SupportBean("E4", 13));
        ArrayAssertionUtil.assertProps(listener.assertOneGetNewAndReset(), fields, new Object[]{13, 12, 13});
    }
    public void testFirstLastIndexedCompiledExpressionBackToEpl() {
        String epl = "select " +
                "last(intPrimitive), " +
                "last(intPrimitive, 1), " +
                "first(intPrimitive), " +
                "first(intPrimitive, 1) " +
                "from SupportBean.win:length(3)";
        EPStatementObjectModel stmt = epService.getEPAdministrator().compileEPL(epl);

        Assert.assertEquals(epl, stmt.toEPL());
    }
